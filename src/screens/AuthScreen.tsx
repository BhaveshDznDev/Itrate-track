import { useEffect } from 'react'
import { useNavigate } from 'react-router-dom'
import { useAuth } from '../hooks/useAuth'
import { Button, Spinner } from '../components/ui'
import { Layers } from 'lucide-react'

export function AuthScreen() {
  const { user, loading, signInWithGoogle } = useAuth()
  const navigate = useNavigate()

  useEffect(() => {
    if (!loading && user) navigate('/')
  }, [user, loading, navigate])

  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-surface-secondary">
        <Spinner />
      </div>
    )
  }

  return (
    <div className="min-h-screen bg-surface-secondary flex items-center justify-center px-4">
      <div className="w-full max-w-sm">
        {/* Logo */}
        <div className="flex flex-col items-center mb-10">
          <div className="w-12 h-12 rounded-2xl bg-action flex items-center justify-center mb-4 shadow-card">
            <Layers className="w-6 h-6 text-white" />
          </div>
          <h1 className="text-2xl font-semibold text-ink font-display">IterateTrack</h1>
          <p className="text-sm text-ink-secondary mt-1 text-center">
            Product lifecycle management for modern PM teams
          </p>
        </div>

        {/* Auth card */}
        <div className="bg-surface rounded-2xl border border-border-subtle shadow-overlay p-8">
          <h2 className="text-lg font-semibold text-ink mb-1">Sign in</h2>
          <p className="text-sm text-ink-secondary mb-6">
            Use your Google account to get started.
          </p>

          <Button
            variant="secondary"
            size="lg"
            className="w-full gap-3"
            onClick={signInWithGoogle}
          >
            <svg className="w-5 h-5" viewBox="0 0 24 24">
              <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/>
              <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/>
              <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/>
              <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/>
            </svg>
            Continue with Google
          </Button>

          <p className="text-xs text-ink-tertiary text-center mt-5">
            By signing in, you agree to our Terms of Service and Privacy Policy.
          </p>
        </div>

        <p className="text-xs text-ink-tertiary text-center mt-6">
          Need to create an org?{' '}
          <button
            onClick={() => navigate('/onboarding')}
            className="text-action underline underline-offset-2"
          >
            Set up your workspace
          </button>
        </p>
      </div>
    </div>
  )
}
